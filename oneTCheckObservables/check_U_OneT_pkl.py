...
#This script checks if total U values reach equilibrium and writes summary file of dipole
...
def check_U_DataFilesForOneT(U_dir):
    U_sortedDataFilesToRead=sort_data_files_by_flushEnd(U_dir)
    len_U=len(U_sortedDataFilesToRead)
    if len_U==0:
        print("no data for U.")
        exit(0)
...
statVec=[]
numDataVec=[]
U_dir=U_dipole_dataDir+"/U/"
print("checking U")
sameUTmp,lagUTmp,pUTmp,statUTmp,numDataPointsU,startingFileInd=check_U_DataFilesForOneT(U_dir)
min_data_num_for_stats=40
...
if (np.min(pVec)>=0.01 or np.max(statVec)<=statThreshhold) and numDataPoints>=min_data_num_for_stats:
...
if numDataPoints<min_data_num_for_stats:
